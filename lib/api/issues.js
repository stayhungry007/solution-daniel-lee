...
Issues.create = async (context) => {
  ...
  try {
    const newIssue = await Issue.create({
      title,
      description,
      created_by: "unknown", // You'll replace this with the actual user
    });
    ...
    await Revision.create({
      ...
      updated_by: "unknown", // Replace with the actual user
    });
...
Issues.update = async (context) => {
  ...
  try {
    ...
    if (Object.keys(changes).length > 0) {
      ...
      issue.updated_by = "unknown"; // Replace with the actual user
      await issue.save();
